...
StructuredUniformBuffer::StructuredUniformBuffer(VulkanApplication *_app,
												 StructuredBufferLayout _uniformLayout, void *data, uint32_t _size, bool _dynamic) : StructuredBuffer(_app, _uniformLayout, data, _size, _dynamic? DYNAMIC_UNIFORM : UNIFORM)
{
	app = _app;
...
bool StructuredUniformBuffer::getDynamic()
{
	return dynamic;
}
...
{
	return uniformLayout.getStride(app, BufferType::UNIFORM);
}